...
class HeteroMGDN(tf.keras.Model):
    def __init__(self, k=10, alpha=0.1, beta=0.9, x_drop_rate=0.0, edge_drop_rate=0.0, 
        ...
        self.x_drop_rate = x_drop_rate  # 输入特征dropout率
        self.edge_drop_rate = edge_drop_rate  # 边的dropout率
        self.z_drop_rate = z_drop_rate  # 输出特征dropout率

        self.x_dropout = tf.keras.layers.Dropout(x_drop_rate)  # 输入特征dropout层
        self.z_dropout = tf.keras.layers.Dropout(z_drop_rate)  # 输出特征dropout层
    ...
    def call(self, inputs, cache=None, training=None, mask=None):
        if len(inputs) == 3:
            x, edge_index, edge_weight = inputs  # 如果有边权重
        else:
            x, edge_index = inputs
            edge_weight = None
        ...
        normed_sparse_adj = LightGCN.norm_adj(edge_index, num_nodes=num_nodes, cache=cache) \
            .dropout(self.edge_drop_rate, training=training)  # 归一化稀疏邻接矩阵并进行dropout

        h = self.x_dropout(x, training=training)  # 输入特征进行dropout
        output = h  # 初始输出为输入特征
        ...
        output = self.z_dropout(output, training=training)  # 输出特征进行dropout
